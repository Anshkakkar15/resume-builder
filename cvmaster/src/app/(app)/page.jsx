...
export default function Home() {
  return (
    <div>
        ...
        <div className="container mx-auto px-3 py-5 sm:px-5 sm:py-7 md:px-7 md:py-9 lg:px-8 xl:px-11 2xl:py-14">
          ...
          <div className="grid grid-cols-1 items-center gap-x-5 gap-y-9 sm:grid-cols-2">
            <div className="mx-auto flex max-w-[450px] flex-col gap-y-3 sm:max-w-full xl:gap-y-6">
              <h1 className="font-popins font-semibold leading-7 text-dark-blue sm:text-lg md:text-xl lg:text-2xl xl:text-3xl 2xl:text-5xl 2xl:leading-[65px]">
                Craft Your Perfect Resume in Minutes!
              </h1>
              <p className="font-popins text-base font-medium leading-6 text-dark-blue md:text-lg xl:text-xl xl:leading-9 2xl:text-2xl">
      ...
      {/* templates */}
      <div id="templates" className="pt-6 sm:pt-9 md:pt-11 lg:pt-14 2xl:pt-16">
        <Carousel
          opts={{ loop: true }}
          plugins={[
            Autoplay({
            ...
            {resumeTemplates?.length >= 1 &&
              resumeTemplates?.map((temlates) => {
                return (
                  <CarouselItem
                    key={temlates.id}
                    className="basis-1/1 xl:basis-1/6"
                  >
                    <Image
                      src={temlates.image}
                      alt="resumes"
